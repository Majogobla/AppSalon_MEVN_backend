...
import { sendEmailVerificatin, sendEmailPasswordReset } from '../emails/authEmailsService.js'
...
const register = async (req, res) => {
  ...
  if(password.trim().length < MIN_PASSWORD_LENGTH) {
    const error = new Error(`La contraseña debe contener almenos ${MIN_PASSWORD_LENGTH} caracteres`)
    return res.status(400).json({ msg: error.message })
  }
  ...
  try {
    ...
    sendEmailVerificatin({ name, email, token })
